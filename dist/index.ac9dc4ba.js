...
function startGame(container) {
    ...
    (function() {
        document.querySelector("#welcome-name-form").addEventListener("submit", handleNameSubmit);
    })();
    ...
    function addNameToLocalStorage(name) {
        ...
        if (players && !players.includes(name)) localStorage.setItem("players", [
            ...players,
            name
        ]);
        else localStorage.setItem("players", [
            name
        ]);
    }
    ...
    function displayNumbersForLevel() {
        for(let i = 0; i < level; i++)alert(generatedNumbers[i]);
    }
    ...
    function gameLoop() {
        ...
        if (verifyLevel()) {
            updateLevel(level + 1);
            gameLoop();
        } else alert(`Your score is: ${level}`);
    }
}
